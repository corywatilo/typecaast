...
- **`isolate`** — render the widget inside an **open shadow root** so it's fully
  isolated from the host page's styles (global CSS, resets, a CSS framework, or a
  design system) that would otherwise bleed in and distort it. Recommended when
  embedding into a page with its own styles. **Client-only** — a shadow root can't be
