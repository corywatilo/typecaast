...
/**
 * A faked playback stub implementing the `Player` interface over a provided
 * `GetStateAt` (no real engine). Drives a clock via rAF (or a timeout fallback)
 * so `<Typecaast>` and the builder shell can play/pause/seek/scrub/step against
 * mocked state. M1-engine replaces this with the real player wrapping the real
 * `getStateAt` — the interface stays identical.
 */
export class MockPlayer implements Player {
  ...
  constructor(getStateAt: GetStateAt, options: MockPlayerOptions) {
...
/** Convenience factory matching the eventual `createPlayer` shape. */
export function createMockPlayer(
  getStateAt: GetStateAt,
  options: MockPlayerOptions,
): Player {
  return new MockPlayer(getStateAt, options);
}
