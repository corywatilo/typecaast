...
- **Timeline track editor** (add/reorder/delete steps; per-step delay/duration/instant overrides; drag to retime).
- **Preview-as-you-go editing.** The live preview is the primary editing surface, not a passive output: play/pause/scrub, step frame-by-frame, loop a selected range, and **adjust timing/pacing inline** — drag a message's delay or typing duration and see it immediately, tweak global WPM/CPS with a live re-pace, nudge a reaction's lag, all without re-rendering from scratch. Editing while playing is supported (seek stays put as you change values).
- **Participant manager** (name, avatar, color, self/app).
- **Skin picker + options form** (auto-generated from the skin's `optionsSchema`), with a **capability/lint panel** showing what the chosen skin will drop.
- **Output controls** — canvas size with **aspect presets** (16:9 / 1:1 / 9:16 / 4:5 / custom) and scale factor, **editable at any time** with live reflow (§9); plus `fit`, `fps`, `seed`.
- **Theme toggle** (light/dark/auto) in the preview so you can check both palettes.
- **Import / paste existing JSON** (round-trip) and **export**: download JSON or copy embed snippet. **No video export from the site in v1** — the builder shows a "render this yourself" snippet (`typecaast render …`), with a "Render for me" entry point reserved for the future paid service.
- Generator output uses **inline data-URL assets** by default (self-contained, shareable); an export toggle switches to `url` mode for users hosting their own images.
- State persisted to URL (shareable links) and localStorage.
